/*
 * arch/arm/mach-tegra/board-enrc2u-power.c
 *
...
static int __init enrc2u_fixed_regulator_init(void)
{
	int i;
...
static int __init enrc2u_gpio_regulator_init(void)
{
	int i, j;
...
static int __init enrc2u_regulators_fixed_gpio_init(void)
{
	int ret;
	...
	ret = enrc2u_fixed_regulator_init();
	if (ret)
		return ret;

	ret = enrc2u_gpio_regulator_init();
	return ret;
}
subsys_initcall_sync(enrc2u_regulators_fixed_gpio_init);

static struct i2c_board_info __initdata enrc2u_regulators[] = {
	{
...
#ifdef CONFIG_PM
static void enrc2u_suspend_work(void)
{
...
static void enrc2u_resume_work(void)
{
...
static int __init enrc2u_gpio_rtc_init(void)
{
	int ret;
...
#ifdef CONFIG_PM
	tps_platform.suspend_work = enrc2u_suspend_work;
	tps_platform.resume_work = enrc2u_resume_work;
#endif
	return 0;
...
void __init enrc2u_tsensor_init(void)
{
	tegra3_tsensor_init(NULL);
}

int __init enrc2u_regulator_init(void)
{
	...
	i2c_register_board_info(4, enrc2u_regulators,
		    ARRAY_SIZE(enrc2u_regulators));

	enrc2u_tsensor_init();
	return 0;
}

static void enrc2u_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void enrc2u_board_resume(int lp_state, enum resume_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_RESUME_AFTER_CPU))
		tegra_console_uart_resume();
}

static struct tegra_suspend_platform_data enrc2u_suspend_data = {
	.cpu_timer	= 600,
	.cpu_off_timer	= 200,
	...
	.corereq_high	= true,
	.sysclkreq_high	= true,
	.board_suspend = enrc2u_board_suspend,
	.board_resume = enrc2u_board_resume,
	.cpu_resume_boost	= 1700000,
	.boost_resume_reason	= 0x80,
};

int __init enrc2u_suspend_init(void)
{
	tegra_init_suspend(&enrc2u_suspend_data);
	return 0;
}

#ifdef CONFIG_TEGRA_EDP_LIMITS
int __init enrc2u_edp_init(void)
{
...
static struct platform_device enrc2u_bpc_mgmt_device = {
	.name		= "tegra-bpc-mgmt",
	.id		= -1,
	.dev		= {
...
void __init enrc2u_bpc_mgmt_init(void)
{
...
#endif
	platform_device_register(&enrc2u_bpc_mgmt_device);
