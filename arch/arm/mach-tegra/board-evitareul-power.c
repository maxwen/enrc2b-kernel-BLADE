/*
 * arch/arm/mach-tegra/board-evitareul-power.c
 *
...
TPS_PDATA_INIT(ldo5, common, 1000, 3300, 0, 0, 0, 0, 2800, 0, 1, 0, 0, 0);
...
static struct regulator_consumer_supply fixed_reg_aud_hsmic_2v85_en_supply[] = {
        REGULATOR_SUPPLY("v_aud_hsmic_2v85", NULL),
};
...
FIXED_REG(2, mhl_3v3_en,     "tps80031_LDOUSB",      TEGRA_GPIO_PY2,  true, 3300, 0, 0);
FIXED_REG(3, lcmio_1v8_en,   "tps80031_VIO", TEGRA_GPIO_PE5,  true, 1800, 1, 0);
FIXED_REG(4, srio_1v8_en,    "tps80031_VIO", TEGRA_GPIO_PY3,  true, 1800, 1, 0);
FIXED_REG(5, cam2_d1v2_en,   "tps80031_VIO", TEGRA_GPIO_PF6,  true, 1200, 0, 0);
FIXED_REG(6, cam_d1v2_en,    "tps80031_VIO", TEGRA_GPIO_PF5,  true, 1200, 0, 0);
FIXED_REG(7, camio_1v8_en,   "tps80031_LDO7",      TEGRA_GPIO_PBB4, true, 1800, 0, 0);
FIXED_REG(8, mhl_1v2_en,     "tps80032_VIO",      TEGRA_GPIO_PE4,  true, 1200, 0, 0);
FIXED_REG(9, aud_hsmic_2v85_en,   NULL,      TEGRA_GPIO_PB2,  true, 2850, 1, 0);
FIXED_REG(10, lcm_3v_en,     NULL,      TEGRA_GPIO_PE2,  true, 3000, 1, 0);
FIXED_REG(11, fuse_src_3v3_en, NULL,    TEGRA_GPIO_PF0,  true, 3300, 0, 0);
FIXED_REG(12, ps_2v85_en,    NULL,      TEGRA_GPIO_PM3,  true, 2850, 1, 0);
...
#define FIXED_REGS_COMMON		\
	...
	ADD_FIXED_REG(aud_hsmic_2v85_en),	\
...
static int __init evitareul_fixed_regulator_init(void)
{
	int i;
...
static int __init evitareul_gpio_regulator_init(void)
{
	int i, j;
...
static int __init evitareul_regulators_fixed_gpio_init(void)
{
	int ret;
	...
	ret = evitareul_fixed_regulator_init();
	if (ret)
		return ret;

	ret = evitareul_gpio_regulator_init();
	return ret;
}
subsys_initcall_sync(evitareul_regulators_fixed_gpio_init);

static struct i2c_board_info __initdata evitareul_regulators[] = {
	{
...
#ifdef CONFIG_PM
static void evitareul_suspend_work(void)
{
...
static void evitareul_resume_work(void)
{
...
static int __init evitareul_gpio_rtc_init(void)
{
	int ret;
...
#ifdef CONFIG_PM
	tps_platform.suspend_work = evitareul_suspend_work;
	tps_platform.resume_work = evitareul_resume_work;
#endif
	return 0;
...
void __init evitareul_tsensor_init(void)
{
	tegra3_tsensor_init(NULL);
}

int __init evitareul_regulator_init(void)
{
	...
	i2c_register_board_info(4, evitareul_regulators,
		    ARRAY_SIZE(evitareul_regulators));

	evitareul_tsensor_init();
	return 0;
}

static void evitareul_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void evitareul_board_resume(int lp_state, enum resume_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_RESUME_AFTER_CPU))
		tegra_console_uart_resume();
}

static struct tegra_suspend_platform_data evitareul_suspend_data = {
	.cpu_timer	= 600,
	.cpu_off_timer	= 200,
	...
	.corereq_high	= true,
	.sysclkreq_high	= true,
	.board_suspend = evitareul_board_suspend,
	.board_resume = evitareul_board_resume,
	.cpu_resume_boost	= 1700000,
	.boost_resume_reason	= 0x80,
};

int __init evitareul_suspend_init(void)
{
	tegra_init_suspend(&evitareul_suspend_data);
	return 0;
}

#ifdef CONFIG_TEGRA_EDP_LIMITS
int __init evitareul_edp_init(void)
{
...
static struct platform_device evitareul_bpc_mgmt_device = {
	.name		= "tegra-bpc-mgmt",
	.id		= -1,
	.dev		= {
...
void __init evitareul_bpc_mgmt_init(void)
{
...
#endif
	platform_device_register(&evitareul_bpc_mgmt_device);
