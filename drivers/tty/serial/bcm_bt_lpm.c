...
struct bcm_bt_lpm {
	...
	void (*request_lpm_on_locked)(struct uart_port *uport);
	void (*request_lpm_off_locked)(struct uart_port *uport);
} bt_lpm;
...
static void set_wake_locked(int wake)
{
	...
	if (wake || bt_lpm.host_wake)
		bt_lpm.request_lpm_off_locked(bt_lpm.uport);
	else
		bt_lpm.request_lpm_on_locked(bt_lpm.uport);
...
void bcm_bt_lpm_exit_lpm(struct uart_port *uport) {
	pr_debug("[BT] %s\n", __func__);
...
static int bcm_bt_lpm_probe(struct platform_device *pdev)
{
	...
	if (bt_lpm.request_lpm_on_locked != NULL) {
	...
	bt_lpm.request_lpm_on_locked = pdata->request_lpm_on_locked;
	bt_lpm.request_lpm_off_locked = pdata->request_lpm_off_locked;
