/*
 * arch/arm/mach-tegra/board-evitareul-sdhci.c
 *
...
static void *evitareul_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init evitareul_init_wifi_mem(void)
{
#ifdef CONFIG_DHD_USE_STATIC_BUF
	int i;
...
static int evitareul_wifi_status_register(void (*callback)(int , void *), void *);

static int evitareul_wifi_reset(int on);
/* HTC_WIFI_START */
//static int evitareul_wifi_power(int on);
//static int evitareul_wifi_set_carddetect(int val);
int evitareul_wifi_power(int on);
int evitareul_wifi_set_carddetect(int val);
int evitareul_wifi_status(struct device *dev);
static int evitareul_wifi_cd;		/* WIFI virtual 'card detect' status */
/* HTC_WIFI_END */

static struct wifi_platform_data evitareul_wifi_control = {
	.set_power      = evitareul_wifi_power,
	.set_reset      = evitareul_wifi_reset,
	.set_carddetect = evitareul_wifi_set_carddetect,
#ifdef CONFIG_DHD_USE_STATIC_BUF
	.mem_prealloc	= evitareul_wifi_mem_prealloc,
#else
	.mem_prealloc	= NULL,
#endif
};
...
static struct platform_device evitareul_wifi_device = {
	.name           = "bcmdhd_wlan",
	.id             = 1,
	.num_resources	= 1,
	.resource	= wifi_resource,
	.dev            = {
		.platform_data = &evitareul_wifi_control,
	},
};
...
int evitareul_wifi_suspend_gpio(void)
{
...
int evitareul_wifi_resume_gpio(void)
{
...
static struct tegra_sdhci_platform_data tegra_sdhci_platform_data0 = {
	.mmc_data = {
		.register_status_notify	= evitareul_wifi_status_register,
...
static struct tegra_sdhci_platform_data tegra_sdhci_platform_data2 = {
	...
	.wp_gpio = -1,
	.power_gpio = -1,
	.suspend_gpiocfg = evitareul_wifi_suspend_gpio,
	.resume_gpiocfg = evitareul_wifi_resume_gpio,
};
...
static int evitareul_wifi_status_register(
...
EXPORT_SYMBOL(evitareul_wifi_set_carddetect);

int evitareul_wifi_sdclk (int enable){
...
/*	
	pr_debug("%s: %d\n", __func__, on);
	gpio_set_value(EVITAUL_WLAN_PWR, on);
	mdelay(100);
	gpio_set_value(EVITAUL_WLAN_RST, on);
	mdelay(200);

	return 0;
*/
}
EXPORT_SYMBOL(evitareul_wifi_power);
/* HTC_WIFI_END */
...
static int __init evitareul_wifi_init(void)
{
	int rc;
	...
	evitareul_init_wifi_mem();
	platform_device_register(&evitareul_wifi_device);
	return 0;
}
bool wifi_isEvitarel = false;
EXPORT_SYMBOL(wifi_isEvitarel);

int __init evitareul_sdhci_init(void)
{
	platform_device_register(&tegra_sdhci_device3);
	platform_device_register(&tegra_sdhci_device2);
	wifi_isEvitarel = true;
	evitareul_wifi_init();
	return 0;
}
