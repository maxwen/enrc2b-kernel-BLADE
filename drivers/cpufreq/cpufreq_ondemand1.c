/*
 *  drivers/cpufreq/cpufreq_ondemand1.c
 *
...
static
#endif
struct cpufreq_governor cpufreq_gov_ondemand1 = {
       .name                   = "ondemand1",
...
static void dbs_chown(void)
{
	...
	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/ignore_nice_load", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown ignore_nice_load error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/io_is_busy", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown io_is_busy error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/powersave_bias", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown powersave_bias error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/sampling_down_factor", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown sampling_down_factor error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/sampling_rate", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown sampling_rate error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/two_phase_freq", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown two_phase_freq error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/up_threshold", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown up_threshold error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/down_differential", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown down_differential error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/touch_poke", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown touch_poke error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/ui_sampling_rate", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown ui_sampling_rate error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/ondemand1/ui_counter", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_ondemand1);
}

static void __exit cpufreq_gov_dbs_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_ondemand1);
}
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_ONDEMAND1
