...
static unsigned s, entries_once = 100;
		...
		loff_t *ppos)
{
	...
	if ((driver->mdmdata_ready[index] & USER_SPACE_LOG_TYPE) && (driver->
		...
		pr_debug("diag: Copy data to user space %d\n",
			 driver->in_busy_hsic_write_on_device);
		if (driver->in_busy_hsic_poll_write_on_device == 1) {

			spin_lock_irqsave(&driver->diagchar_lock, flags);

			if (driver->write_ptr_mdm->length + ret > IN_POLL_BUF_SIZE) {
				driver->pkt_size_to_user = IN_POLL_BUF_SIZE - ret;
				driver->write_ptr_mdm->length -= driver->pkt_size_to_user;
			} else {
				driver->pkt_size_to_user = driver->write_ptr_mdm->length;
				driver->write_ptr_mdm->length = 0;
			}

			memcpy(driver->buf_poll_to_user,
					driver->buf_poll_in_hsic, driver->pkt_size_to_user);

			if (driver->write_ptr_mdm->length > 0) {
				DIAG_INFO("static poll byte shifting: %d\n",
					driver->write_ptr_mdm->length);
				/* shift the remain data from begining of poll */
				memcpy(driver->buf_poll_in_hsic,
					driver->buf_poll_in_hsic + driver->pkt_size_to_user,
					driver->write_ptr_mdm->length);
			} else {
				/* static poll was empty here */
				driver->in_busy_hsic_poll_write_on_device = 0;
			}
			...
			/*Copy the actual data being passed*/
			COPY_USER_SPACE_OR_EXIT(buf+ret,
					*(driver->buf_poll_to_user),
					 driver->pkt_size_to_user);
			pr_debug("diag: data copied\n");
		}
		if (driver->hsic_ch)
			queue_work(driver->diag_hsic_wq, &driver->diag_read_hsic_work);
		...
		size_t count, loff_t *ppos)
{
	...
	if (pkt_type == USER_SPACE_LOG_TYPE) {
		...
		if (driver->hsic_ch) {
			memcpy(buf_9k, buf, payload_size);
			diag_bridge_write(buf_9k, payload_size);
			queue_work(driver->diag_hsic_wq, &driver->diag_read_hsic_work);
		}
#endif
		if (diag9k_debug_mask)
